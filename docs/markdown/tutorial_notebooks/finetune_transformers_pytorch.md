...
This functionality can guess a model's configuration, tokenizer and architecture just passing in the model's name.

Using the AutoClasses functionality when can reuse the code on a large number of transformers models!

This notebook is designed to use a pretrained transformers model and finetune it on the classification task on a 
specific dataset (in this case your desired dataset).
...
Since I am using PyTorch to finetune our transformers models any knowledge on PyTorch is very useful.
...
The only modifications will be in reading in the dataset inside the PyTorch Dataset class under **Dataset and DataLoader** tab. The DataLoader will return a dictionary of inputs in a batch format so that it can be fed to the model using the statement: `outputs = model(**batch)`. As long as this statement holds, the rest of the code will work!
...
* `batches` - will be used as the batch size during training. The larger the batch the more RAM / GPU memory it will take. When running into Cuda Out Of Memory (OOM) errors just decrease batch size until the errors is solved. If the model is very large and GPU very weak it can lead up to `batches = 1`.
...
This notebook will cover transformers finetune for the classification task. I will use the well known moview reviews positive - negative labeled [Large Movie Review Dataset](https://ai.stanford.edu/~amaas/data/sentiment/).

The description provided on the Stanford website:

*This is a dataset for binary sentiment classification containing substantially more data than previous benchmark datasets. We provide a set of 25,000 highly polar movie reviews for training, and 25,000 for testing. There is additional unlabeled data for use as well. Raw text and already processed bag of words formats are provided. See the README file contained in the release for more details.*

**Why this dataset?** I believe is an easy to understand and use dataset for classification and I always enjoy working with sentiment data.
...
Now let’s do some coding! We will go through each coding cell in the notebook and describe what it does, what's the code, and what should be the output.

I made this format to be easy to follow if you decide to run each cell in your own python notebook.
...
* `batches` - Number of batches - depending on the max sequence length and GPU memory. For 512 sequence length batch of 10 works without cuda memory issues. For small sequence length can try batch of 32 or higher.
* `max_length` - Pad or truncate text sequences to a specific length. I will set it to 60 tokens since it doesn't need a lot of text to figure out the sentiment and training runs a lot faster.
* `device` - Look for gpu to use. Will use `cpu` by default if no gpu found.
* `model_name_or_path` - Name of transformers model - will use already pretrained model. Path of transformer model - will load your own model from local disk. I always like to start off with `bert-base-cased`.
    Details of `bert-base-cased`:
    * 12-layer, 768-hidden, 12-heads, 109M parameters.
    * Trained on cased English text.
* `labels_ids` - Dictionary of labels and their id - this will be used to convert. String labels to number ids.
* `n_labels` - How many labels are we using in training. This is used to decide size of classification head.
...
We keep all Classes and functions that will be used in this notebook for a clean structure:
...
tokenizer that transforms text into numbers and gets it into a nice format to be fed to the model.
...
In this class I only need to read in content of each file, use **fix_text** to fix any Unicode problems and keep track 
of positive and negative sentiments.

I will append all texts and labels in lists that later I will feed to tokenizer and label_ids to transform everything 
into numbers.

There are three main parts of this class:

* **__init__()** where we read in the dataset and transform text and labels into numbers.

* **__len__()** where we need to return the number of examples we read in. This is used when using 
**len(MovieReviewsDataset())** to know how many examples we have in our dataset.

* **__getitem()** always take as an input a int that represents which example from our examples to return from our 
dataset. If a value of 3 is passed, we will return the example form our dataset at position 3. It needs to return an 
object with the format that can eb fed to our model. Luckily our tokenizer does that for us and returns a dictionary 
of variables ready to be fed to the model like this `model(**inputs)`.
...
I created this function to perform a full pass through the DataLoader object created from our Dataset that was created 
from our **MovieReviewsDataset** class. This is basically one epoch train through the entire dataset.

The *dataloader* of type *torch.utils.data.dataloader.DataLoader* taking the object created from *MovieReviewsDataset* 
of type *torch.utils.data.dataloader.DataLoader* and putting each example in batches. This way we can feed our model 
batches of data!

The optimizer and scheduler are very common in PyTorch. They are required to update the parameters of our model and 
update our learning rate during training. Obviously is a lot more than that but I won't go into details. This can 
actually be a huge rabbit hole since A LOT happens behind these functions that we don't need to worry. Thank you PyTorch!
...
I implemented this function in a very simialr way as train but without the parameters update and the backward and 
gradient decent part. We don't need to do all those VERY computationally intensive tasks because we only care about our
 model's predictions.
...
Loading the three essential parts of pretrained transformers: configuration, tokenizer and model. We also need to load 
model to the device we're planning to use (GPU / CPU).

Since we use the AutoClass functionality from Hugging Face we only need to worry about the model's name as input and 
rest of it is done by us!
...
This is where we go back to *MovieReviewsDataset* class and create out dataset variables. Since we have data partitioned 
for both train and test we will create a Dataset and DataLoader object for train and test. ONLY for simplicity I will use 
...
Create optimizer and scheduler used by PyTorch in training.
...
I've been using this code for a while now and I feel it got to a point where is very nicely documented and easy to follow.

Of course is easy to follow for me because I built it. That is why I really need your feedback to improve in my future
tutorial!
